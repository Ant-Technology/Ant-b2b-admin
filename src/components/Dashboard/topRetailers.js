...
export const TopRetailers = () => {
  ...
  const fetchDataTopRetailers = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${API}/top-retailers?limit=${selectedBadge}&date_range=${timeRange}`,
        config
      );
      if (data) {
        setData(data);
  ...
  const handleBadgeClick = async (badgeValue) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${API}/top-retailers?limit=${badgeValue}&date_range=${timeRange}`,
        config
      );
      if (data) {
        setData(data);
  ...
  const handleTimeRangeChange = async (value) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${API}/top-retailers?limit=${selectedBadge}&date_range=${value}`,
        config
      );
      if (data) {
        setData(data);
  ...
  return (
    ...
    >
      ...
      <Group position="apart">
        <Title order={4}>Top Retailers</Title>
        <Select
          data={[
                ...
                <Box style={{ display: "flex", flex: 1, minWidth: 0 }}>
                  ...
                  <Box style={{ marginLeft: "auto", flexShrink: 0 }}>
                    <Text size="sm" color="green" weight={700}>
                      {formatNumber(item.grand_total_price)}
                    </Text>
                  </Box>
                </Box>
              </Group>
