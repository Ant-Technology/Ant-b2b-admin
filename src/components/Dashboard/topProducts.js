...
export const TopProducts = () => {
  ...
  const fetchDataTopRetailers = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${API}/top-products?limit=${selectedBadge}&date_range=${timeRange}`,
        config
      );
      if (data) {
        setData(data.topProducts);
      }
    } catch (error) {
  ...
  const handleBadgeClick = async (badgeValue) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${API}/top-products?limit=${badgeValue}&date_range=${timeRange}`,
        config
      );
      if (data) {
        setData(data.topProducts);
  ...
  const handleTimeRangeChange = async (value) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${API}/top-products?limit=${selectedBadge}&date_range=${value}`,
        config
      );
      if (data) {
        setData(data.topProducts);
            ...
            >
              <Group spacing="sm" style={{ flex: 1 }}>
                <Box style={{ display: "flex", flex: 1, minWidth: 0 }}>
                  ...
                  >
                    {item.product_sku?.sku} - {item.product_sku?.product?.name}
                  </Text>
                  <Box
                    style={{
                      marginLeft: "20px", // Adjusts the spacing
                      flexShrink: 0,
                    }}
                  >
                    <Text size="sm" color="green" weight={700}>
                      {formatNumber(
                        item.total_quantity * parseFloat(item.product_sku.price)
                      )}
                    </Text>
                  </Box>
                </Box>
