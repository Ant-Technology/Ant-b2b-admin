...
const RetailerReport = () => {
  ...
  const fetchData = async (page) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API}/reports/retailers?period=custom&startDate=2024-01-01&endDate=2025-01-31`,
        config
      );
      if (response.data) {
        setLoading(false);
        setDrivers(response.data.purchaseHistory);
        setSortedData(response.data.purchaseHistory);
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        ...
        }&startDate=${startDate}&endDate=${endDate}&product=${product}`,
        config
      );
  ...
  const rows = sortedData?.map((row) => (
    <Fragment key={row.id}>
      <tr>
        <td style={{ width: "80px" }}>{row.retailer}</td>
        <td style={{ width: "120px" }}>{row.retailerAddress}</td>
        <td style={{ width: "20px" }}>{row.quantity}</td>
        <td style={{ width: "30px" }}>{row.unitPrice}</td>
        <td style={{ width: "70px" }}>{row.subtotal}</td>
        <td style={{ width: "100px" }}>{row.retailerPhone}</td>
        <td style={{ width: "100px" }}>{row.retailerEmail}</td>
  ...
  return (
    <div style={{ width: "98%", margin: "auto" }}>
      ...
      <div>
        ...
        {(timeRange || selectedEndDate || selectedStartDate || product) && (
          <div
            style={{
              display: "flex",
          ...
          >
            <thead>
              <tr style={{ backgroundColor: "#F1F1F1" }}>
                ...
                <Th>
                  <span className={classes.thh}>Category</span>
                </Th>
                <Th>
